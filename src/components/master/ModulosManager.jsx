...
export default function ModulosManager() {
  ...
  const load = async () => {
    const list = await base44.entities.ModuloSentinela.list();
    setModulos(list);
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {["cidadao", "agente", "gestao", "ia", "saude", "defesa_civil"].map(cat => {
        const list = modulos.filter(m => m.categoria === cat);
        ...
        return (
          <div key={cat} className="rounded-2xl border border-border/60 bg-card p-4">
            ...
            <div className="grid md:grid-cols-2 gap-2">
              {list.map(m => (
                <div key={m.id} className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${m.ativo ? "border-border/60" : "border-border/30 opacity-50"}`}>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${m.ativo ? "bg-primary/15" : "bg-muted"}`}>
                    <Package className={`w-3.5 h-3.5 ${m.ativo ? "text-primary" : "text-muted-foreground"}`} />
                  </div>
                  ...
                  <button onClick={() => toggleAtivo(m)} className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${m.ativo ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
